...
function checkCraft() {
  ...
  recipeLoop: for (let i = 0; i < recipeKeys.length; i++) {
    ...
    // const nbrOfItemsInTable = nbrOfItemsInTable(craft);

    const nbrOfIgredients = null; //The number of ingredients in the shapeless recipe
    if (typeof mcItem["ingredients"] !== "undefined") {
      nbrOfIgredients = mcItem["ingredients"].lenght;
    }
    ...
    if (mcItem["type"] === "minecraft:crafting_shaped") {
      ...
      rowLoop: for (let rowNbr = 0; rowNbr < recipePattern.lenght; rowNbr++) {
        ...
        itemLoop: for (let j = 0; j < row.length; j++) {
          ...
          if (rowItem === inventory["craftTable"][rowNbr][j]["item"]) {
            ...
            if (
              ...
              break recipeLoop; //try with the next recipe
            }
          } else {
            isMatch = false; //if not =>
            break recipeLoop; //try with the next recipe
